...
const ViewAnalyticsModal = ({ open, onClose, employeeId, employeeName }) => {
  ...
  // Socket.IO listener for real-time updates (uses shared socket instance)
  useEffect(() => {
    ...
    // Set up event listener (socket is already connected by AuthContext)
    ...
    return () => {
      ...
      // Don't disconnect socket - it's shared and managed by AuthContext
    };
  ...
  const calculateKPIs = () => {
    ...
    // Probation calculation removed - use /api/analytics/probation-tracker endpoint for accurate probation data
    // Company policy: 6 calendar months from joining date, extended only by approved leaves
    ...
    for (
      ...
      if (log) {
        ...
        if (breaksArray.length > 0) {
          totalBreakMinutes = breaksArray.reduce((sum, breakItem) => {
              ...
              <Grid container spacing={3}>
                ...
                <Grid item xs={12} md={4}>
                  <Card className="metric-card probation-card">
                    <CardContent>
                      <Box className="probation-header">
                        <Typography variant="caption" className="probation-subtext" style={{ color: '#666', fontStyle: 'italic' }}>
                          Probation data available in Probation Tracker (Admin → Analytics)
                        </Typography>
                      </Box>
                      <Typography variant="body2" style={{ marginTop: 8, color: '#999', fontSize: '0.75rem' }}>
                        Company policy: 6 calendar months from joining, extended only by approved leaves
                      </Typography>
                    </CardContent>
                  </Card>
                </Grid>
