...
const LeavesPage = () => {
    ...
    const leaveSummaryCards = useMemo(() => [
        { key: 'sick', label: 'Sick Leave', value: Number(leaveBalances.sick ?? 0), icon: <SickIcon />, variant: 'sick' },
        ...
        return (
            <div className="leaves-page">
                ...
                <div className="leave-summary-row">
                    {[1, 2, 3].map((slot) => (
                        <div key={`summary-${slot}`} className="leave-summary-card loading">
                            <Skeleton variant="text" width={100} height={18} style={{ marginBottom: 10 }} />
                ...
                <div className="leave-grid">
                    {[1, 2, 3].map((slot) => (
                        <div key={`card-${slot}`} className="leave-card loading-card">
                            <Skeleton variant="text" width={180} height={24} style={{ marginBottom: 16 }} />
        ...
        <div className="leaves-page">
            ...
            <div className="leave-header-card">
                <div className="leave-header">
                    <div className="header-left">
                        <Typography variant="h4" className="leave-header-text">
                            Leave Management
                        </Typography>
                    </div>
                    ...
                    <div className="header-right">
                        {yearEndFeatureEnabled && isPermanentEmployee && (
                            <IconButton
                                onClick={(e) => setYearEndMenuAnchor(e.currentTarget)}
                                className="year-end-menu-button"
                                title="Year-End Leave Options"
                            >
                                <MoreVertIcon />
                            </IconButton>
                        )}
                        <Button className="apply-leave-button" onClick={handleOpenModal}>
                            + Apply Leave
                        </Button>
                    </div>
                </div>
            ...
            <div className="leave-summary-row">
                {leaveSummaryCards.map(card => (
                    <div key={card.key} className={`leave-summary-card ${card.variant}`}>
                        <div className="leave-summary-card-top">
                            <span className="leave-summary-card-label">{card.label}</span>
                            <span className="leave-summary-card-icon">{card.icon}</span>
                        </div>
                        <div className="leave-summary-card-count">{card.value}</div>
                    </div>
                ))}
            </div>

            <div className="leave-grid">
                <div className="leave-card application-card">
                    <div className="leave-card-title">Application Requests</div>
                    <div className="leave-card-body">
                        {myRequests.length === 0 ? (
                            <div className="requests-empty-state">
                                <CalendarTodayIcon className="requests-empty-icon" />
                                <div className="requests-empty-state-title">No Leave Requests</div>
                                <p className="requests-empty-state-text">
                                    You haven't submitted any leave requests yet. Click Apply Leave to get started.
                                </p>
                                <Button className="empty-apply-btn" onClick={handleOpenModal}>
                                    + Apply Leave
                                </Button>
                            </div>
                        ) : (
                ...
                <div className="leave-card holidays-card">
                        ...
                        <ul className="holiday-list">
                            {holidays && holidays.length > 0 ? (
                                holidays.map((holiday, idx) => {
                                    ...
                                    return (
                                        <li key={holiday._id || idx} className="holiday-item">
                                            ...
                                            {isTentative && <span className="holiday-badge">Tentative</span>}
                                        </li>
                                    );
                                })
                            ) : (
                                <li className="holiday-item empty-byte">
                                    <span className="holiday-content">
                                        <span className="holiday-name">No holidays scheduled</span>
                                    </span>
                                </li>
                            )}
                        </ul>
                ...
                <div className="leave-card saturday-card">
                    <div className="leave-card-title">Saturday Schedule</div>
                    <div className="leave-card-body">
                        {saturdaySchedule.length === 0 ? (
                            <div className="requests-empty-state">
                                <AccessTimeIcon className="requests-empty-icon" />
                                <div className="requests-empty-state-title">No Schedule Available</div>
                                <p className="requests-empty-state-text">
                                    Saturday schedule details will appear here.
                                </p>
                            </div>
                        ) : (
                            <ul className="saturday-list">
                                {saturdaySchedule.map(({ date, status }, idx) => {
                                    const isWorking = status === 'Working';
                                    const isLeave = status.includes('Approved');
                                    const dayLabel = date.toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' });
                                    const badgeText = isLeave ? 'On Leave' : (isWorking ? 'Working' : 'Off');
                                    const subtitleText = isLeave ? status : (isWorking ? 'Working Day' : 'Holiday');
                                    const statusClass = isLeave ? 'leave' : (isWorking ? 'working' : 'off');
                                    return (
                                        <li key={`${date.toISOString()}-${idx}`} className="saturday-item">
                                            <span className="holiday-icon">
                                                <AccessTimeIcon />
                                            </span>
                                            <span className="saturday-content">
                                                <span className="saturday-date">{dayLabel}</span>
                                                <span className="holiday-date">{subtitleText}</span>
                                            </span>
