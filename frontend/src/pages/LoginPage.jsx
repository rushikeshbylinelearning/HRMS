...
const LoginPage = () => {
    ...
    const checkExistingSession = async () => {
        try {
            ...
            if (response.data && response.data.email) {
                console.log('[LoginPage] ✅ Existing session valid - redirecting to dashboard');
    ...
    const handleSSOAuthentication = async (ssoToken) => {
        ...
        const processedToken = sessionStorage.getItem('sso_processed_token');
        ...
        try {
            ...
            // Mark token as processed to prevent reprocessing
            sessionStorage.setItem('sso_processed_token', ssoToken);
            
            // Set authorization header for all future API calls IMMEDIATELY
            api.defaults.headers.common['Authorization'] = `Bearer ${data.token}`;
            
            console.log('[SSO Debug] ✅ AMS JWT stored in sessionStorage (ams_token and token)');
