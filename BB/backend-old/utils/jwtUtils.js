...
function getPrivateKey() {
  try {
    ...
    // Guard against file system errors - never throw globally
    console.error('[JWT Utils] Private key access failed:', error.message);
...
function getPublicKey() {
  try {
    ...
    // Guard against file system errors - never throw globally
    console.error('[JWT Utils] Public key access failed:', error.message);
    throw error; // Re-throw for caller to handle
  }
}
...
function sign(payload, options = {}) {
  ...
  try {
    ...
    // Safe fallback to HS256 if RSA keys are missing
    const fallbackSecret = process.env.JWT_SECRET;
    if (!fallbackSecret) {
      console.error('[JWT Debug] ❌ No JWT_SECRET available for HS256 fallback');
      throw new Error(`JWT signing failed: ${error.message}. No RSA keys and no JWT_SECRET configured.`);
    }
    ...
    try {
      ...
      console.error('[JWT Debug] ❌ HS256 fallback also failed:', fallbackError.message);
      throw new Error(`JWT signing failed completely: RS256 (${error.message}) and HS256 (${fallbackError.message})`);
    }
  }
}
...
function validateRS256Configuration() {
  try {
    ...
    // Check local key files - wrap in try-catch to prevent startup crashes
    try {
      const privateKey = getPrivateKey();
      const publicKey = getPublicKey();
      ...
      console.error('   Server will continue with HS256 fallback if JWT_SECRET is available');
    }
    ...
    console.log('✅ HS256 fallbacks: AVAILABLE');
    console.log('================================');
  } catch (error) {
    // Never throw from validation - just log warnings
    console.error('❌ JWT Configuration validation encountered error:', error.message);
    console.error('   Server will continue but JWT operations may fail');
  }
}
