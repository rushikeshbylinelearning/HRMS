...
describe('Special 10 AM - 7 PM Shift: Early Clock-In Logout Calculation', () => {
    ...
    describe('Edge Cases', () => {
        ...
        describe('New Policy Validation Tests', () => {
            ...
            it('Scenario: No paid break, unpaid 25 min (unpaid always extends logout)', () => {
                const clockInTime = createISTDate(2024, 1, 15, 9, 0);
                const sessions = [{ startTime: clockInTime.toISOString() }];
                const attendanceLog = {
                    paidBreakMinutesTaken: 0,
                ...
                // Unpaid break: 25 min (always adds full duration) = 25 min
                // Result: 18:00 + 25 = 18:25
                // Special shift minimum: max(18:25, 19:00) = 19:00
                expect(logoutTime).toBe('19:00');
            });
            
            it('Scenario: No paid break, unpaid 50 min (unpaid always extends logout)', () => {
                const clockInTime = createISTDate(2024, 1, 15, 9, 0);
                const sessions = [{ startTime: clockInTime.toISOString() }];
                const attendanceLog = {
                    paidBreakMinutesTaken: 0,
                ...
                // Base: 09:00 + 9 hours = 18:00
                // Paid break: 0 min = 0 min
                // Unpaid break: 50 min (always adds full duration) = 50 min
                // Result: 18:00 + 50 = 18:50
                // Special shift minimum: max(18:50, 19:00) = 19:00
                expect(logoutTime).toBe('19:00');
            });
            ...
            it('Scenario: Paid 45 + unpaid 20 (both extend logout)', () => {
                ...
                // Paid break: 45 min (>30, excess = 15 min) = 15 min
                // Unpaid break: 20 min (always adds full duration) = 20 min
                // Result: 19:00 + 15 + 20 = 19:35
                expect(logoutTime).toBe('19:35');
            });
            
            it('Scenario: No paid break, unpaid 60 min (unpaid always extends logout beyond 7 PM)', () => {
                const clockInTime = createISTDate(2024, 1, 15, 10, 0);
                const sessions = [{ startTime: clockInTime.toISOString() }];
                const attendanceLog = {
                    paidBreakMinutesTaken: 0,
                    unpaidBreakMinutesTaken: 60,
                    penaltyMinutes: 0
                };
                ...
                // Base: 10:00 + 9 hours = 19:00
                // Paid break: 0 min = 0 min
                // Unpaid break: 60 min (always adds full duration) = 60 min
                // Result: 19:00 + 60 = 20:00
                expect(logoutTime).toBe('20:00');
            });
        });
    });
