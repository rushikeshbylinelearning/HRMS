...
function ssoTokenAuth(SSO_CONFIG) {
  ...
  return async (req, res, next) => {
    try {
      ...
      // Defensive guard: ensure query exists
      const token = req.query && req.query.sso_token;
      ...
      // If SSO is not configured, redirect to SSO portal - defensive check
      ...
      // Verify the SSO token using the new verification utility - with error containment
      let decoded;
      try {
        decoded = await ssoVerification.verifyToken(token);
      } catch (verifyError) {
        console.error('[SSOAuth] Token verification failed:', verifyError.message);
        throw verifyError;
      }
      
      // Extract user claims (now includes appEmail/appPassword if available) - defensive
      const userClaims = ssoVerification.extractUserClaims(decoded);
      
      // Use appEmail for user lookup (primary), fallback to SSO email - defensive guards
      const lookupEmail = userClaims.appEmail || userClaims.email;
      
      if (!lookupEmail) {
        console.error('[SSOAuth] No email found in SSO token claims');
        throw new Error('SSO token missing email information');
      }
      ...
      // Find or create user using appEmail (not SSO email) - with error containment
      let user;
      try {
        ...
        console.error('[SSOAuth] Database query failed:', dbError.message);
      ...
      if (!user) {
        ...
        // Auto-provision new user using appEmail - with error containment
        console.log(`[SSOAuth] Auto-provisioning new user: ${lookupEmail}`);
        
        // Hash password if provided in token - defensive approach
        ...
        if (userClaims.appPassword) {
          try {
            ...
            console.error('[SSOAuth] Password hashing failed:', hashError.message);
            // Continue without password - SSO-only user
        ...
        try {
          ...
          console.error('[SSOAuth] User creation failed:', createError.message);
          throw new Error('Failed to create new user account');
        }
      } else {
        console.log(`[SSOAuth] Found existing AMS user: ${lookupEmail}`);
        
        // Verify app password if provided in token - defensive approach
        ...
        // Update user data from SSO if needed - defensive approach
        try {
          const needsUpdate = 
          ...
          console.error('[SSOAuth] User update failed:', updateError.message);
          // Continue without update - not critical
        }
      }

      // Create session - defensive approach
      try {
        if (!req.session) {
          console.warn('[SSOAuth] Session not available, skipping session creation');
        } else {
          req.session.user = {
            id: user._id,
            name: user.fullName,
        ...
        console.error('[SSOAuth] Session creation failed:', sessionError.message);
        // Continue without session - token-based auth is primary
      }

      console.log(`✅ SSO Login: ${lookupEmail}`);

      // Redirect to intended route - defensive approach
      const returnUrl = (req.query && req.query.return_url) || '/dashboard';
      return res.redirect(returnUrl);

    } catch (err) {
      console.error('❌ SSO middleware error:', err.message);
      // Never let SSO middleware crash the app - always redirect to login
      const redirectUrl = process.env.NODE_ENV === 'production'
        ? 'https://sso.bylinelms.com/login'
        : 'http://localhost:3000/login';
      return res.redirect(redirectUrl);
