...
async function emitTeaBreakStarted(teaPayload, initiatedByUserId = null) {
  try {
    ...
    // Admin announcement channel (active break banner) — all authenticated users
    ...
    for (const userId of clockedInIds) {
      if (initiatorId && userId === initiatorId) continue;

      // Targeted socket delivery for clocked-in employees (timer + desktop hooks)
      io.to(`user_${userId}`).emit("tea_break_started", normalizedPayload);

      try {
        await NewNotificationService.notifyTeaBreakStarted(
          userId,
        ...
        console.error(`[Announcements] tea break notification failed for ${userId}:`, notifErr.message);
      }
    }

    const connectedCount = (await io.fetchSockets()).length;
    console.log(
      `[Announcements] tea_break_started (${connectedCount} socket(s), ${clockedInIds.length} clocked-in, ${notified} notified, initiator excluded: ${initiatorId || "none"}) announcement ${announcementId}, remainingSeconds=${timing.remainingSeconds}`
    );

    // Web Push — OS notifications when the app tab is in the background
    try {
      const { notifyTeaBreakStartedPush } = require("../services/webPushService");
      await notifyTeaBreakStartedPush(clockedInIds, {
        teaBreakType: normalizedPayload.teaBreakType,
        announcementId,
        initiatedByUserId: initiatorId,
      });
    } catch (pushErr) {
      console.error("[Announcements] tea break Web Push failed:", pushErr.message);
    }
  } catch (err) {
